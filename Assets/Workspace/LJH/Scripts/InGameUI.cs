...
public class InGameUI : MonoBehaviour
{
    ...
    void Start()
    {
        // 지금 이부분 널레퍼런스떠서 안돌아감
        // 초기화
        hpBar.value = _player._hp;          // 체력 게이지 초기화
        hpBar.maxValue = Mathf.CeilToInt(_player._hp);      // 체력 게이지 최대값 설정
        healthPointText.text = $"{hpBar.maxValue} / {hpBar.maxValue}";  // 체력 텍스트 초기화
